...
export const createProductController = async (req, res) => {
  try {
    ...
    switch (true) {
      ...
      case photo && photo.size > 1000000 || photo1.size > 1000000 :
        return res
          .status(500)
...
export const productPhoto1Controller = async (req, res) => {
  try {
    const product = await productModel.findById(req.params.pid).select("photo1");
...
export const sortController = async (req, res) => {
  try {
    ...
    // Fetch the sorted products from the database
    const products = await productModel.find().sort(sortOptions).exec();
